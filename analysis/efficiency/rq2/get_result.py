...
def calculate_optimizer_stats_and_merge(problem_df, SEED_RANGE):
    optimizer_stats = problem_df.groupby('optimizer_name')['efficiency'].agg(
        mean='mean',
    ...
    pivot_df = problem_df.pivot(index='optimizer_name', columns='seed', values='efficiency')
...
if __name__ == "__main__":
    csv_path = "result.csv"
    main_result_folder = "output/rq2"
    new_result_folder = "output/optimizer_rank_sub_files"
    SEED_RANGE = range(101, 131)
    MAX_RANK = 32

    ranking_csv = "output/rq2_ranking_results.csv"
    ranking_with_cat_csv = "output/rq2_ranking_with_categories.csv"
    optimizer_rank_csv = "output/rq2_optimizer_rank_by_domain_with_category.csv"
    category_summary_csv = "output/rq2_optimizer_category_rank_summary.csv"
    ...
    try:
        ...
        required_columns = ['optimizer_name', 'efficiency', 'seed', 'problem_name']
        ...
        df = df.dropna(subset=['seed', 'efficiency'])
